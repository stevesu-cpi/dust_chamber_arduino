...
const int TIME_ON = 3; // On time in seconds per RELAY
...
int total_cycle = 60 / ((float)((float)TIME_ON * 8 + (float)TIME_OFF)/60) * (float)TOTAL_TEST_HOURS; // Calculate total number of cycles: 60min/hr * total time per cycle (cyc/mins) * total target test hours
